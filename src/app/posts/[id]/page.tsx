...
export default async function SharedPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const post = await prisma.post.findUnique({ where: { id }, include: { author: { select: { name: true, username: true, image: true } }, _count: { select: { likes: true, comments: true, shares: true } }, mentions: { include: { mentionedUser: { select: { id: true, name: true, username: true, image: true } } } } } });
  if (!post) notFound();
