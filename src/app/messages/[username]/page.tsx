...
export default function IndividualMessagePage() {
    ...
    try {
      ...
      if (silent && pollingReadyRef.current) {
        ...
        if (incomingCount > 0 || unreadCount > 0) {
          if (atBottom) {
            // The newest message is already in view; keep the chat clean and mark it seen.
            setNewMessageCount(0);
      ...
      if (!silent && firstConversationLoadRef.current === false) {
        // The first load has already established the baseline. Nothing else is needed here.
      }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => void loadConversation(conversationId, true), 1200);
  ...
  function startTouchMessage(event: TouchEvent, message: Message) {
    ...
    if (touch) {
      longPressTimerRef.current = window.setTimeout(() => {
        ...
        setSwipingId(null);
        handleMessageContextMenu({ preventDefault: () => {}, clientX: touch.clientX, clientY: touch.clientY } as MouseEvent, message);
      }, LONG_PRESS_MS);
    }
  }
  ...
  async function deleteMessage(message: Message, mode: "me" | "both") {
    ...
    try {
      const response = await fetch(`/api/messages/${encodeURIComponent(conversationId)}/${encodeURIComponent(message.id)}`, {
        method: "DELETE",
        credentials: "include",
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-7 rounded-[2rem] border border-white/[0.08] bg-white/[0.025] p-5 sm:p-8">
            ...
            <div className="relative mt-7">
              ...
              {results.length > 0 && <div className="absolute left-0 right-0 top-14 z-20 overflow-hidden rounded-2xl border border-white/[0.10] bg-[#0b0b0b] shadow-2xl">{results.map((result) => <Link key={result.id} href={`/messages/${encodeURIComponent(result.username)}`} className="flex items-center gap-3 border-b border-white/[0.06] px-4 py-3 last:border-0 hover:bg-white/[0.04]"><span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-red-600/10 text-sm font-bold text-red-300">{result.image ? <img src={result.image} alt="" className="h-full w-full object-cover" /> : result.name.charAt(0).toUpperCase()}</span><span><span className="block text-sm font-semibold">{result.name}</span><span className="block text-xs text-white/30">@{result.username}</span></span></Link>)}</div>}
            </div>
          </div>
        </div>
        <MobileNav />
  ...
  return (
    <main className="min-h-screen bg-black px-3 py-4 pb-28 text-white sm:px-6 sm:py-8">
      <div className="mx-auto flex h-[calc(100dvh-7rem)] min-h-[520px] max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-white/[0.08] bg-white/[0.025] shadow-[0_25px_80px_rgba(0,0,0,0.45)]">
        <header className="z-30 flex shrink-0 items-center gap-3 border-b border-white/[0.07] bg-[#080808]/95 px-3 py-3.5 backdrop-blur-xl sm:px-6 sm:py-4">
          <button type="button" onClick={() => router.push("/messages")} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] text-white/55 hover:border-red-400/20 hover:bg-red-500/[0.05] hover:text-white" aria-label="Back to messages">←</button>
          <Link href={`/users/${encodeURIComponent(user?.username || username)}`} className="flex min-w-0 flex-1 items-center gap-3 rounded-xl py-1 text-left hover:bg-white/[0.03]">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/[0.08] bg-red-600/10 text-sm font-bold text-red-300">{user?.image ? <img src={user.image} alt={title} className="h-full w-full object-cover" /> : (user?.name || username).charAt(0).toUpperCase()}</span>
            <span className="min-w-0"><span className="block truncate text-[15px] font-bold text-white">{title}</span><span className="block truncate text-xs text-white/35">@{user?.username || username}</span></span>
          </Link>
          <div className="hidden items-center gap-2 md:flex"><LiveSocialActions /></div>
        ...
        {chatStatus !== "ACCEPTED" ? (
          ...
          <>
            ...
            {contextMenu && <div ref={contextMenuRef} role="menu" className="fixed z-[2147483647] min-w-[200px] rounded-xl border border-white/[0.12] bg-[#0b0b0b] p-1.5 shadow-2xl" style={{ left: contextMenu.x, top: contextMenu.y }} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
