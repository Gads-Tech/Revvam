...
export default function PostComposer({ onCreated }: { onCreated: (post: PostData) => void }) {
  ...
  return <form onSubmit={createPost} className="rounded-[2rem] border border-white/[0.08] bg-white/[0.025] p-5 backdrop-blur-2xl sm:p-6">
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2">
      ...
      <button type="button" onClick={() => imageInput.current?.click()} className="rounded-full border border-white/[0.08] bg-white/[0.03] px-3.5 py-2 text-xs font-semibold text-white/55 hover:text-white">📷 Photo</button>
      <button type="button" onClick={() => videoInput.current?.click()} className="rounded-full border border-white/[0.08] bg-white/[0.03] px-3.5 py-2 text-xs font-semibold text-white/55 hover:text-white">🎥 Video</button>
    ...
    {tagged.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{tagged.map((user) => <button key={user.id} type="button" onClick={() => setTagged((current) => current.filter((item) => item.id !== user.id))} className="rounded-full border border-red-400/15 bg-red-500/[0.06] px-2.5 py-1 text-[10px] text-red-300">@{user.username} ×</button>)}</div>}
