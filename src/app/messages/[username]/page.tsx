...
export default function IndividualMessagePage() {
  ...
  async function sendMedia(file: File, type: "IMAGE" | "VIDEO" | "AUDIO", durationMs?: number) {
    ...
    try {
      ...
      const res = await fetch("/api/messages/" + encodeURIComponent(conversationId), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content: type === "IMAGE" ? "📷 Photo" : type === "VIDEO" ? "🎥 Video" : "🎙️ Voice message", messageType: type, mediaUrl: u.mediaUrl, mediaMimeType: u.mediaMimeType, mediaSize: u.mediaSize, durationMs: durationMs ?? null }) });
