    ...
    <main className="min-h-screen overflow-x-hidden bg-[#030303] text-white">
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl px-4 pb-32 pt-6 sm:px-6 sm:pt-10 lg:px-8">
        ...
        <section className="mt-6 rounded-[2rem] border border-white/[0.08] bg-white/[0.025] p-5 sm:p-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/25">Your garage</p><h2 className="mt-1 text-xl font-bold">Vehicles on Revvam</h2><p className="mt-1 text-sm text-white/30">Manage the vehicles people can explore on your public profile.</p></div><Link href="/profile/driver" className="inline-flex w-fit rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-2.5 text-xs font-semibold text-white/55 transition hover:border-red-500/20 hover:text-white">Manage garage</Link></div>
          {vehicles.length > 0 ? <div className="mt-6 grid gap-3 md:grid-cols-2">{vehicles.map((vehicle) => { const image = vehicle.image || vehicle.photos[0]?.url; return <Link key={vehicle.id} href={`/profile/cars/${vehicle.id}`} className="group flex min-w-0 items-center gap-4 rounded-2xl border border-white/[0.07] bg-black/20 p-3.5 transition hover:-translate-y-0.5 hover:border-red-500/20 hover:bg-red-500/[0.035]"><div className="relative h-16 w-20 shrink-0 overflow-hidden rounded-xl border border-white/[0.07] bg-white/[0.025]">{image ? <img src={image} alt={`${vehicle.make} ${vehicle.model}`} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center text-2xl opacity-30">🚗</div>}{vehicle.isFeatured && <span className="absolute bottom-1 left-1 rounded bg-red-600/85 px-1.5 py-0.5 text-[7px] font-bold uppercase">Featured</span>}</div><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-semibold text-white/85 group-hover:text-white">{vehicle.make} {vehicle.model}</h3><p className="mt-1 text-xs text-white/25">{vehicle.year ?? "Year unknown"} · {vehicle.type ?? "Vehicle"}</p></div><span className="text-white/20 transition group-hover:text-red-300">→</span></Link>; })}</div> : <div className="mt-6 rounded-2xl border border-dashed border-white/[0.09] px-6 py-10 text-center text-sm text-white/30">No vehicles yet.</div>}
        </section>
...
function HeaderAction({ href, label, icon, count, compact = false }: { href: string; label: string; icon: string; count: number; compact?: boolean }) {
