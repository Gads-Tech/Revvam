...
export default function IndividualMessagePage() {
  ...
  async function loadTarget() {
    ...
    try {
      ...
      const chatResponse = await fetch(`/api/users/${encodeURIComponent(username)}/chat-request`, {
        credentials: "include",
        cache: "no-store",
      });
  ...
  async function loadConversation(id: string, silent = false) {
    ...
    try {
      ...
      const response = await fetch(
        `/api/messages/${encodeURIComponent(id)}?${markRead ? "markRead=1&" : ""}_=${Date.now()}`,
        {
          credentials: "include",
          cache: "no-store",
      ...
      if (silent) {
        ...
        // Never auto-scroll during polling. New messages are deliberately announced
        // with the floating arrow so the user keeps control of their scroll position.
        if (incomingUnread > 0) setNewMessageCount(incomingUnread);
      }
  ...
  useEffect(() => {
    if (!isComposer || search.trim().length < 1) {
      setResults([]);
      return;
    }
  ...
  function handleMessageContextMenu(event: MouseEvent, message: Message) {
    ...
    const menuWidth = 200;
    const menuHeight = canDeleteBoth ? 132 : 94;
  ...
  function moveTouchMessage(event: TouchEvent) {
    const touch = event.touches[0];
    if (!touch) return;
    if (Math.abs(touch.clientX - touchStartXRef.current) > 12 || Math.abs(touch.clientY - touchStartYRef.current) > 12) {
      cancelLongPress();
    }
  }
  ...
  async function deleteMessage(message: Message, mode: "me" | "both") {
    ...
    // Client-side guard mirrors the server rule: only the sender can use
    // Delete for everyone, and only during the first two minutes.
    const mine = message.senderId === currentUserId;
    ...
    try {
      const response = await fetch(
        `/api/messages/${encodeURIComponent(conversationId)}/${encodeURIComponent(message.id)}`,
        {
          method: "DELETE",
          credentials: "include",
  ...
  async function sendRequest() {
    ...
    try {
      const response = await fetch(`/api/users/${encodeURIComponent(user.username)}/chat-request`, {
        method: "POST",
        credentials: "include",
  ...
  async function sendMessage() {
    ...
    try {
      const response = await fetch(`/api/messages/${encodeURIComponent(conversationId)}`, {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content: clean, replyToId: replyId }),
      });
      const data = await response.json();
  ...
  function handleMessageScroll() {
    ...
    // If the user manually reaches the bottom, treat the new messages as seen.
      ...
      <main className="min-h-screen bg-black px-4 py-6 pb-28 text-white sm:px-6 sm:py-8">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-7 rounded-[2rem] border border-white/[0.08] bg-white/[0.025] p-5 sm:p-8">
            <p className="text-xs uppercase tracking-[0.22em] text-red-400/70">New conversation</p>
            <h1 className="mt-2 text-3xl font-black tracking-[-0.045em]">Find someone on Revvam</h1>
            <p className="mt-3 text-sm leading-6 text-white/35">Search for a driver or car enthusiast, then send a chat request with a short message.</p>
            <div className="relative mt-7">
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search username..." className="h-12 w-full rounded-2xl border border-white/[0.10] bg-black/30 px-4 text-sm text-white outline-none placeholder:text-white/20 focus:border-red-400/30" />
              {results.length > 0 && (
                <div className="absolute left-0 right-0 top-14 z-20 overflow-hidden rounded-2xl border border-white/[0.10] bg-[#0b0b0b] shadow-2xl">
                  {results.map((result) => (
                    <Link key={result.id} href={`/messages/${encodeURIComponent(result.username)}`} className="flex items-center gap-3 border-b border-white/[0.06] px-4 py-3 last:border-0 hover:bg-white/[0.04]">
                      <span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-red-600/10 text-sm font-bold text-red-300">
                        {result.image ? <img src={result.image} alt="" className="h-full w-full object-cover" /> : result.name.charAt(0).toUpperCase()}
                      </span>
                      <span><span className="block text-sm font-semibold">{result.name}</span><span className="block text-xs text-white/30">@{result.username}</span></span>
                    </Link>
                  ))}
                </div>
              )}
  ...
  return (
    <main className="min-h-screen bg-black px-3 py-4 pb-28 text-white sm:px-6 sm:py-8">
      <div className="mx-auto flex h-[calc(100dvh-7rem)] min-h-[520px] max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-white/[0.08] bg-white/[0.025]">
        <header className="z-30 flex shrink-0 items-center gap-3 border-b border-white/[0.07] bg-[#080808] px-3 py-3.5 sm:px-6 sm:py-4">
          <button type="button" onClick={() => router.push("/messages")} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] text-white/55 hover:text-white" aria-label="Back to messages">←</button>

          <Link href={`/users/${encodeURIComponent(user?.username || username)}`} className="flex min-w-0 flex-1 items-center gap-3 rounded-xl py-1 text-left hover:bg-white/[0.03]">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/[0.08] bg-red-600/10 text-sm font-bold text-red-300">
              {user?.image ? <img src={user.image} alt={title} className="h-full w-full object-cover" /> : (user?.name || username).charAt(0).toUpperCase()}
            </span>
            <span className="min-w-0">
              <span className="block truncate text-[15px] font-bold text-white">{title}</span>
              <span className="block truncate text-xs text-white/35">@{user?.username || username}</span>
            </span>
          </Link>
        ...
        {chatStatus !== "ACCEPTED" ? (
          <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto p-5 sm:p-10">
            <div className="w-full max-w-xl rounded-[2rem] border border-red-400/15 bg-red-500/[0.035] p-6 sm:p-8">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-red-400/70">Private chat request</p>
              <h1 className="mt-2 text-2xl font-black">Say hello to {title}</h1>
              <p className="mt-3 text-sm leading-6 text-white/35">Send a short message with your request. They must accept before the private chat opens.</p>
              {chatStatus === "PENDING_SENT" ? (
                <div className="mt-7 rounded-2xl border border-white/[0.08] bg-black/20 p-5"><p className="text-sm font-semibold text-white/70">Request sent</p><p className="mt-2 text-sm text-white/30">Waiting for @{user?.username} to accept.</p></div>
              ) : chatStatus === "PENDING_RECEIVED" ? (
                <div className="mt-7 rounded-2xl border border-white/[0.08] bg-black/20 p-5"><p className="text-sm font-semibold text-white/70">They already sent you a request.</p><Link href="/profile/notifications" className="mt-4 inline-flex rounded-xl border border-red-400/20 bg-red-500/10 px-4 py-2.5 text-xs font-semibold text-red-200">Review request</Link></div>
              ) : (
                <>
                  <textarea value={requestMessage} onChange={(event) => setRequestMessage(event.target.value)} maxLength={1000} rows={5} placeholder="Write a message with your request..." className="mt-7 w-full resize-none rounded-2xl border border-white/[0.10] bg-black/30 px-4 py-3 text-sm leading-6 text-white outline-none placeholder:text-white/20 focus:border-red-400/30" />
                  <div className="mt-3 flex justify-end"><button type="button" onClick={sendRequest} disabled={requestBusy || !requestMessage.trim()} className="rounded-xl border border-red-400/20 bg-red-500/10 px-5 py-2.5 text-xs font-semibold text-red-200 disabled:opacity-40">{requestBusy ? "Sending..." : "Send chat request"}</button></div>
                </>
              )}
            </div>
          </div>
          ...
          <>
            <div ref={messagesContainerRef} onScroll={handleMessageScroll} className="relative min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-4 py-6 sm:px-7">
              {messages.length === 0 ? (
                <p className="py-16 text-center text-sm text-white/25">No messages yet. Say hello.</p>
              ) : (
                  ...
                  return (
                    <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                      <div
                        ...
                        onTouchStart={(event) => startTouchMessage(event, message)}
                        onTouchMove={moveTouchMessage}
              ...
              {newMessageCount > 0 && (
                <button type="button" onClick={jumpToNewMessages} className="absolute bottom-4 right-4 z-20 flex items-center gap-2 rounded-full border border-red-400/25 bg-[#0b0b0b]/95 px-3 py-2 text-xs font-bold text-white shadow-[0_10px_35px_rgba(0,0,0,0.75)] backdrop-blur-xl transition hover:border-red-400/45 hover:bg-red-500/10" aria-label={`Jump to ${newMessageCount} new messages`}>
                  <span className="flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-[11px] text-white">↓</span>
                  <span>{newMessageCount > 99 ? "99+" : newMessageCount} new</span>
                </button>
              )}
            </div>

            {contextMenu && (
              <div ref={contextMenuRef} role="menu" className="fixed z-[2147483647] min-w-[200px] rounded-xl border border-white/[0.12] bg-[#0b0b0b] p-1.5 shadow-2xl" style={{ left: contextMenu.x, top: contextMenu.y }} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
                ...
                {contextMenu.message.senderId === currentUserId && Date.now() - new Date(contextMenu.message.createdAt).getTime() <= DELETE_FOR_BOTH_MS && (
            ...
            <div className="shrink-0 border-t border-white/[0.07] bg-[#080808] p-3 sm:p-5">
              ...
              <div className="flex items-end gap-2">
                <textarea ref={composerRef} value={content} onChange={(event) => setContent(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendMessage(); } }} rows={1} placeholder="Write a message..." aria-label="Write a message" className="min-h-11 max-h-32 flex-1 resize-none rounded-2xl border border-white/[0.10] bg-black/30 px-4 py-3 text-sm text-white outline-none placeholder:text-white/20 focus:border-red-400/30" />
                <button type="button" onClick={() => void sendMessage()} disabled={sending || !content.trim()} className="min-h-11 shrink-0 rounded-2xl border border-red-400/20 bg-red-500/[0.12] px-5 text-sm font-semibold text-red-300 disabled:opacity-40">{sending ? "..." : "Send"}</button>
              </div>
