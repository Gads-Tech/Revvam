...
export default function IndividualMessagePage() {
  ...
  async function loadTarget() {
    ...
    try {
      ...
      if (chatResponse.ok && chatData.success) {
        ...
        setRequestId(chatData.requestId || "");
  ...
  async function loadConversation(id: string, silent = false) {
    ...
    try {
      ...
      if (!silent) setError(loadError instanceof Error ? loadError.message : "Unable to load chat.");
    }
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => loadConversation(conversationId, true), 2500);
  ...
  async function sendRequest() {
    ...
    try {
      ...
      setError(sendError instanceof Error ? sendError.message : "Unable to send chat request.");
    } finally {
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-7 rounded-[2rem] border border-white/[0.08] bg-white/[0.025] p-5 sm:p-8">
            ...
            <div className="relative mt-7">
              <input autoFocus value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search username..." className="h-12 w-full rounded-2xl border border-white/[0.10] bg-black/30 px-4 text-sm text-white outline-none placeholder:text-white/20 focus:border-red-400/30" />
              {results.length > 0 && <div className="absolute left-0 right-0 top-14 z-20 overflow-hidden rounded-2xl border border-white/[0.10] bg-[#0b0b0b] shadow-2xl">{results.map((result) => <Link key={result.id} href={`/messages/${encodeURIComponent(result.username)}`} className="flex items-center gap-3 border-b border-white/[0.06] px-4 py-3 last:border-0 hover:bg-white/[0.04]"><span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-red-600/10 text-sm font-bold text-red-300">{result.image ? <img src={result.image} alt="" className="h-full w-full object-cover" /> : result.name.charAt(0).toUpperCase()}</span><span className="min-w-0"><span className="block truncate text-sm font-semibold">{result.name}</span><span className="block truncate text-xs text-white/30">@{result.username}</span></span></Link>)}</div>}
            </div>
            <div className="mt-8 rounded-2xl border border-white/[0.07] bg-black/20 p-5 text-center text-sm text-white/25">Type a username above to see people you can message.</div>
          </div>
        </div>
        <MobileNav />
      </main>
        ...
        {chatStatus !== "ACCEPTED" ? (
          <div className="flex flex-1 items-center justify-center p-5 sm:p-10">
            <div className="w-full max-w-xl rounded-[2rem] border border-red-400/15 bg-red-500/[0.035] p-6 sm:p-8">
              ...
              {chatStatus === "PENDING_SENT" ? <div className="mt-7 rounded-2xl border border-white/[0.08] bg-black/20 p-5"><p className="text-sm font-semibold text-white/70">Request sent</p><p className="mt-2 text-sm leading-6 text-white/30">Waiting for @{user?.username} to accept your chat request.</p></div> : chatStatus === "PENDING_RECEIVED" ? <div className="mt-7 rounded-2xl border border-white/[0.08] bg-black/20 p-5"><p className="text-sm font-semibold text-white/70">They already sent you a request.</p><Link href="/profile/notifications" className="mt-4 inline-flex rounded-xl border border-red-400/20 bg-red-500/10 px-4 py-2.5 text-xs font-semibold text-red-200">Review request</Link></div> : chatStatus === "DECLINED_BY_TARGET" ? <div className="mt-7 rounded-2xl border border-white/[0.08] bg-black/20 p-5 text-sm text-white/35">Chat requests from you are currently unavailable for this user.</div> : <><textarea value={requestMessage} onChange={(event) => setRequestMessage(event.target.value)} maxLength={1000} rows={5} placeholder="Write a message with your request..." className="mt-7 w-full resize-none rounded-2xl border border-white/[0.10] bg-black/30 px-4 py-3 text-sm leading-6 text-white outline-none placeholder:text-white/20 focus:border-red-400/30" /><div className="mt-3 flex items-center justify-between gap-3"><span className="text-[11px] text-white/20">{requestMessage.length}/1000</span><button type="button" onClick={sendRequest} disabled={requestBusy || !requestMessage.trim()} className="rounded-xl border border-red-400/20 bg-red-500/10 px-5 py-2.5 text-xs font-semibold text-red-200 disabled:opacity-40">{requestBusy ? "Sending..." : "Send chat request"}</button></div></>}
            </div>
          </div>
        ) : (
          <>
            <div className="flex-1 space-y-3 overflow-y-auto px-4 py-6 sm:px-7">
              {messages.length === 0 ? <p className="py-16 text-center text-sm text-white/25">No messages yet. Say hello.</p> : messages.map((message) => { const mine = message.sender.id !== user?.id; return <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6 ${mine ? "rounded-br-md bg-red-600/20 text-white" : "rounded-bl-md bg-white/[0.06] text-white/75"}`}>{message.content}</div></div>; })}
            </div>
