...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const conversation = await prisma.conversation.findUnique({
      ...
      include: {
        members: { where: { userId: { not: user.id } }, include: { user: { select: { id: true, name: true, username: true, image: true } } } },
      },
    });

    return NextResponse.json({ success: true, messages, otherUser: conversation?.members[0]?.user ?? null });
  } catch (error) {
