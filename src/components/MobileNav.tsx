...
export default function MobileNav() {
  ...
  useEffect(() => {
    ...
    const closeOnOutsideTap = (event: PointerEvent) => {
      ...
      if (!target?.closest("[data-revvam-profile-menu], [data-revvam-profile-toggle]")) {
    ...
    <>
      ...
      <div className={`mobile-nav-root fixed inset-x-0 bottom-0 z-[2147483646] px-3 pb-[max(8px,env(safe-area-inset-bottom))] transition-transform duration-300 ease-out ${mobileNavOpen ? "translate-y-0" : "translate-y-full"}`} style={{ pointerEvents: mobileNavOpen ? "auto" : "none", touchAction: "manipulation", isolation: "isolate", zIndex: 2147483646 }}>
        <div className="relative mx-auto w-full max-w-[520px]">
          {profileMenuOpen && mobileNavOpen && <div data-revvam-profile-menu className="absolute bottom-[74px] right-0 z-[2147483647] w-[210px] overflow-hidden rounded-2xl border border-white/[0.12] bg-[#080808] p-1.5 shadow-[0_18px_50px_rgba(0,0,0,0.9)]" style={{ pointerEvents: "auto", touchAction: "manipulation" }}>
            <Link href="/profile" onClick={() => setProfileMenuOpen(false)} className="flex min-h-11 items-center rounded-xl px-3 text-sm font-semibold text-white/80 transition-colors active:bg-white/[0.08] active:text-white" style={{ touchAction: "manipulation" }}><span className="mr-3 text-white/50">◉</span>View profile</Link>
            <Link href="/profile/notifications" onClick={() => setProfileMenuOpen(false)} className="relative flex min-h-11 items-center rounded-xl px-3 text-sm font-semibold text-white/80 transition-colors active:bg-white/[0.08] active:text-white" style={{ touchAction: "manipulation" }}><span className="mr-3 text-red-400/80">●</span>Notifications</Link>
            <Link href="/messages" onClick={() => setProfileMenuOpen(false)} className="relative flex min-h-11 items-center rounded-xl px-3 text-sm font-semibold text-white/80 transition-colors active:bg-white/[0.08] active:text-white" style={{ touchAction: "manipulation" }}><span className="mr-3 text-white/50">✉</span>Messages</Link>
            <Link href="/profile/avatar" onClick={() => setProfileMenuOpen(false)} className="flex min-h-11 items-center rounded-xl px-3 text-sm font-semibold text-white/80 transition-colors active:bg-white/[0.08] active:text-white" style={{ touchAction: "manipulation" }}><span className="mr-3 text-white/50">◌</span>Profile photo</Link>
            <div className="my-1 border-t border-white/[0.08]" />
            <button type="button" onClick={handleLogout} disabled={loggingOut} className="flex min-h-11 w-full items-center rounded-xl px-3 text-left text-sm font-semibold text-red-300 transition-colors active:bg-red-500/[0.12] active:text-red-200 disabled:opacity-50" style={{ touchAction: "manipulation" }}><span className="mr-3 text-red-400/80">↪</span>{loggingOut ? "Logging out…" : "Log out"}</button>
          </div>}
          ...
          <nav aria-label="Mobile navigation" className="relative z-[2147483647] flex h-16 w-full flex-row items-stretch overflow-hidden rounded-[20px] border border-white/[0.12] bg-[#080808] p-1 shadow-[0_10px_40px_rgba(0,0,0,0.9)]" style={{ pointerEvents: "auto", touchAction: "manipulation", zIndex: 2147483647 }}>
            ...
            <button data-revvam-profile-toggle type="button" onClick={() => setProfileMenuOpen((open) => !open)} aria-expanded={profileMenuOpen} aria-label="Profile menu" className={`flex h-full min-w-0 flex-1 touch-manipulation flex-col items-center justify-center rounded-[15px] px-0.5 py-1 transition-all active:scale-[0.96] ${pathname?.startsWith("/profile") || profileMenuOpen ? "bg-red-500/[0.10] text-red-300" : "text-white/50 active:bg-white/[0.05]"}`} style={{ WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}><span className="flex h-5 w-5 items-center justify-center rounded-full border border-current/20 bg-white/[0.05] text-[10px] font-bold leading-none">◉</span><span className="mt-1 w-full truncate text-center text-[9px] font-medium leading-none tracking-tight">Profile</span></button>
          </nav>
        </div>
      </div>
    </>, document.body
