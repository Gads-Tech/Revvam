  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-black text-white">
      ...
      <section className="relative z-10 px-4 pb-14 pt-36 sm:px-6 sm:pt-40 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-4xl">
            ...
            <p className="mt-7 max-w-2xl text-base leading-7 text-white/45 sm:text-lg sm:leading-8">Explore real posts, builds and people from the automotive community. You can look around freely — create an account when you&apos;re ready to join the conversation.</p>
      ...
      <section id="community" className="relative z-10 border-y border-white/[0.06] bg-white/[0.015] px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.28em] text-red-400/70">Community feed</p><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">What people are posting</h2><p className="mt-2 max-w-xl text-sm leading-6 text-white/30">This is Revvam in motion. Browse the public conversation before you decide to jump in.</p></div><Link href="/signup" className="inline-flex w-fit rounded-full border border-white/[0.10] bg-white/[0.04] px-4 py-2.5 text-xs font-semibold text-white/55 transition hover:border-red-400/20 hover:bg-red-500/[0.06] hover:text-white">Create an account to post</Link></div>
          {posts.length === 0 ? <GlassCard className="p-10 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-red-500/10 text-2xl">✦</div><h3 className="mt-5 text-xl font-semibold">The road is yours.</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-white/35">The community is getting started. Join Revvam and be one of the first people to post.</p><Link href="/signup" className="mt-6 inline-flex rounded-full border border-red-400/20 bg-red-600/10 px-5 py-2.5 text-sm font-semibold text-red-200 hover:bg-red-600/20">Join now</Link></GlassCard> : <div className="grid gap-4 lg:grid-cols-2">{posts.map((post) => <article key={post.id} className="overflow-hidden rounded-[1.75rem] border border-white/[0.08] bg-black/45 shadow-2xl shadow-black/20 transition hover:border-white/[0.14]"><div className="p-5 sm:p-6"><Link href={`/users/${encodeURIComponent(post.author.username)}`} className="group flex items-center gap-3"><Avatar image={post.author.image} fallback={post.author.username} /><div className="min-w-0"><p className="truncate text-sm font-semibold group-hover:text-red-300">{post.author.name}</p><p className="truncate text-xs text-white/25">@{post.author.username} · {formatDate(post.createdAt)}</p></div></Link><p className="mt-5 whitespace-pre-wrap text-[15px] leading-7 text-white/70">{post.content}</p></div>{post.image && <div className="max-h-[520px] overflow-hidden border-y border-white/[0.06] bg-black"><img src={post.image} alt="Community post" className="mx-auto max-h-[520px] w-full object-contain" /></div>}<div className="flex items-center gap-2 border-t border-white/[0.06] p-3 sm:p-4"><Link href="/login" className="flex-1 rounded-xl px-3 py-2.5 text-center text-xs font-semibold text-white/35 transition hover:bg-white/[0.05] hover:text-red-300">♡ Like</Link><Link href="/login" className="flex-1 rounded-xl px-3 py-2.5 text-center text-xs font-semibold text-white/35 transition hover:bg-white/[0.05] hover:text-white">○ Comment</Link><Link href="/login" className="flex-1 rounded-xl px-3 py-2.5 text-center text-xs font-semibold text-white/35 transition hover:bg-white/[0.05] hover:text-white">↗ Share</Link></div></article>)}</div>}
        </div>
      </section>
      <section id="how-it-works" className="relative z-10 border-t border-white/[0.06] px-4 py-16 pb-32 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl"><div className="rounded-[2rem] border border-white/[0.08] bg-gradient-to-br from-red-600/[0.10] via-white/[0.025] to-transparent p-7 sm:p-10"><p className="text-[10px] font-bold uppercase tracking-[0.28em] text-red-400/70">Ready to take part?</p><div className="mt-3 flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><h2 className="text-3xl font-black tracking-tight sm:text-4xl">Look around. Then join the community.</h2><p className="mt-3 text-sm leading-7 text-white/35 sm:text-base">Browsing is open to everyone. To like posts, comment, publish your own posts or build your Revvam profile, create an account or log in.</p></div><div className="flex flex-wrap gap-3"><Link href="/signup" className="inline-flex h-12 items-center rounded-2xl border border-red-400/25 bg-red-600/20 px-6 text-sm font-bold hover:bg-red-500/30">Join Now →</Link><Link href="/login" className="inline-flex h-12 items-center rounded-2xl border border-white/[0.10] bg-white/[0.04] px-6 text-sm font-semibold text-white/60 hover:bg-white/[0.07] hover:text-white">Log in</Link></div></div></div><p className="mt-8 text-center text-xs text-white/15">Revvam · Your car. Your community.</p></div>
      </section>
    </main>
  );
}

function Avatar({ image, fallback }: { image?: string | null; fallback: string }) { return <span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-red-600/15 text-xs font-bold text-red-300">{image ? <img src={image} alt="" className="h-full w-full object-cover" /> : fallback.charAt(0).toUpperCase()}</span>; }
