    ...
    <>
        ...
        <div className="relative mx-auto w-full max-w-[520px]">
          {profileMenuOpen && mobileNavOpen && (
            <div className="absolute bottom-[74px] right-0 z-[2147483647] w-[190px] overflow-hidden rounded-2xl border border-white/[0.12] bg-[#080808] p-1.5 shadow-[0_18px_50px_rgba(0,0,0,0.9)]" style={{ pointerEvents: "auto", touchAction: "manipulation" }}>
              ...
              <button type="button" onClick={handleLogout} disabled={loggingOut} className="flex min-h-11 w-full items-center rounded-xl px-3 text-left text-sm font-semibold text-red-300 transition-colors active:bg-red-500/[0.12] active:text-red-200 disabled:opacity-50" style={{ touchAction: "manipulation" }}>
                <span className="mr-3 text-red-400/80">↪</span>{loggingOut ? "Logging out…" : "Log out"}
              </button>
            </div>
          )}
          ...
          <nav aria-label="Mobile navigation" className="relative z-[2147483647] flex h-16 w-full flex-row items-stretch overflow-hidden rounded-[20px] border border-white/[0.12] bg-[#080808] p-1 shadow-[0_10px_40px_rgba(0,0,0,0.9)]" style={{ pointerEvents: "auto", touchAction: "manipulation", zIndex: 2147483647 }}>
            ...
            <button type="button" onClick={() => setProfileMenuOpen((open) => !open)} aria-expanded={profileMenuOpen} aria-label="Profile menu" className={`flex h-full min-w-0 flex-1 touch-manipulation flex-col items-center justify-center rounded-[15px] px-0.5 py-1 transition-all active:scale-[0.96] ${pathname?.startsWith("/profile") || profileMenuOpen ? "bg-red-500/[0.10] text-red-300" : "text-white/50 active:bg-white/[0.05]"}`} style={{ WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>
              <span className="flex h-5 w-5 items-center justify-center rounded-full border border-current/20 bg-white/[0.05] text-[10px] font-bold leading-none">◉</span>
      ...
      {!mobileNavOpen && (
        <button type="button" onClick={() => setMobileNavOpen(true)} aria-label="Show navigation" className="mobile-nav-root fixed bottom-[max(12px,env(safe-area-inset-bottom))] right-4 z-[2147483647] flex h-6 w-16 items-center justify-center rounded-full border border-white/[0.14] bg-[#080808]/95 shadow-[0_8px_30px_rgba(0,0,0,0.8)] backdrop-blur-xl transition-all active:bg-red-500/15" style={{ pointerEvents: "auto", touchAction: "manipulation", zIndex: 2147483647, WebkitTapHighlightColor: "transparent" }}>
          <span aria-hidden="true" className="block h-0.5 w-7 rounded-full bg-white/60" />
        </button>
      )}
    </>,
    document.body
...
function MobileNavItem({ icon, label, href, onClick, active = false }: { icon: string; label: string; href?: string; onClick?: () => void; active?: boolean }) {
  ...
  const content = (
    <>
      <span className="flex h-5 items-center justify-center text-[14px] leading-none">{icon}</span>
