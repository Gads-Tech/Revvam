...
export async function PATCH(request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  if (!chatRequest || chatRequest.recipientId !== currentUser.id) {
    return NextResponse.json({ success: false, error: "Chat request not found." }, { status: 404 });
  }
  if (chatRequest.status !== "PENDING") {
    return NextResponse.json({ success: false, error: "This request has already been handled." }, { status: 409 });
  }
