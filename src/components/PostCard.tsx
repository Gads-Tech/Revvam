        ...
        <div className="flex max-h-[88dvh] w-full max-w-md min-h-0 flex-col overflow-hidden rounded-[2rem] border border-white/[0.10] bg-[#090909] shadow-[0_25px_80px_rgba(0,0,0,0.75)]">
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden p-4 sm:p-5">
            ...
            <div className="mt-3 min-h-0 flex-1 overflow-y-auto overscroll-y-contain pr-1 [scrollbar-width:thin] touch-pan-y" style={{ WebkitOverflowScrolling: "touch" }}>
              {loadingFriends ? <p className="px-2 py-5 text-center text-xs text-white/25">Loading your friends...</p> : <div className="space-y-2">{filteredFriends.map((friend) => <ShareUser key={friend.id} friend={friend} sendingTo={sendingTo} onSend={sendPostToUser} />)}{searchOnlyUsers.map((friend) => <ShareUser key={friend.id} friend={friend} sendingTo={sendingTo} onSend={sendPostToUser} />)}{!filteredFriends.length && !searchOnlyUsers.length && <p className="px-2 py-8 text-center text-xs text-white/25">{shareSearch ? "No users found." : "No conversations yet."}</p>}</div>}
            </div>
          </div>
        </div>
      </div>}
