  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/[0.07] bg-black/80 backdrop-blur-2xl">
      <div className="mx-auto flex h-[76px] max-w-[1480px] items-center justify-between px-5 sm:px-6 xl:px-8">
        <Link href="/home" aria-label="Revvam Discover" className="shrink-0"><Logo className="h-10 w-auto scale-[1.55] transform-gpu sm:h-11" /></Link>
        <nav aria-label="Primary navigation" className="hidden h-full items-center gap-8 md:flex">
          {links.map((link) => {
            const active = link.href === "/home" ? pathname === "/home" : link.href === "/map" ? pathname === "/map" : link.href === "/messages" ? pathname.startsWith("/messages") : link.href === "/profile/mechanic" ? pathname.startsWith("/profile/mechanic") : false;
            return <Link key={link.label} href={link.href} className={`relative flex h-full items-center text-sm font-semibold transition ${active ? "text-white after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-red-500" : "text-white/40 hover:text-white"}`}>{link.label}</Link>;
          })}
        </nav>
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <Link href="/profile" aria-label={profile ? `Open @${profile.username} profile` : "Open profile"} className="flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.025] px-1.5 py-1.5 transition hover:border-red-400/25">
            ...
            {profile && <span className="hidden max-w-28 text-left lg:block"><span className="block truncate text-xs font-semibold">{profile.name}</span><span className="block truncate text-[9px] text-white/30">@{profile.username}</span></span>}
          </Link>
        </div>
      </div>
    </header>
